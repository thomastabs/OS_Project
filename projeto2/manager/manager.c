...
int send_request_list_box(char *server_name, char *client_name) {
    ...
    // since this variable can switch its size quite often, we set its space
    // with 1312
    ...
    for (size_t i = 0; i < strlen(response); i += 58) {
        fprintf(stdout, "%.1s %.32s %.8s %.8s %.8s\n", response + i + 1,
                response + i + 2, response + i + 34, response + i + 42,
                response + i +
                    50); // 58 being the size of each response of the request
    }
    return 0;
}
