...
int send_pub_request(int server_pipe, char* client_name, char* box) {
    ...
    // with this type of format it will be easy to separate each value from each other
    // so the pub request will have for example this format: '\001|../manager1|box'
    
    // with this for loop we can check if the box in question is, in fact, in the system
    // and within our custom array that is responsible to hold the arrays
    ...
    // if the response read which was made by the server is -1, then it returns -1
    int response;
...
int main(int argc, char **argv) {
    ...
    if (send_pub_request(server_pipe, pub_pipename, box_name) == -1){
        ...
        // right after reducing the number of publishers connected to the box in question
        // since the pub session will end
        for(int i = 0; i< BOX_NAME; i++){
